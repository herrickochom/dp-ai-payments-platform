...
-- Clear beneficiary IDs are used only inside the controlled resolution joins.
-- The ordinary Silver output exposes canonical tokens, never internal IDs.

select distinct
    payment.source_system,
    payment.transaction_id,
    payment.end_to_end_id,
    transaction_identity.beneficiary_token as transaction_beneficiary_token,
    payment.sacco_id as transaction_sacco_id,
    payment.agent_id as transaction_agent_id,
    loan.loan_id,
    loan_link.beneficiary_token as loan_beneficiary_token,
    loan.sacco_id as loan_sacco_id,
    loan.business_plan_id,
    beneficiary.beneficiary_token as matched_beneficiary_token,
    ...
    case
        ...
        when loan.beneficiary_id is not null and loan_link.beneficiary_token is null then 'BENEFICIARY_NOT_MATCHED'
        when payment.beneficiary_id is not null
          and transaction_identity.beneficiary_token is distinct from loan_link.beneficiary_token
            then 'BENEFICIARY_MISMATCH'
...
left join {{ ref('vlt_pdm_beneficiary_identity') }} transaction_identity
  on payment.beneficiary_id = transaction_identity.beneficiary_id

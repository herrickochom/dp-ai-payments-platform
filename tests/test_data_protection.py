...
def test_registry_valid_and_single_sourced():
    ...
    assert set(p1["classifications"]) == {"PUBLIC", "INTERNAL", "CONFIDENTIAL", "RESTRICTED"}
    ...
    required = {"beneficiary_name", "nin", "phone", "email", "beneficiary_id",
                "x_beneficiary_sa", "x_wallet_reference", "debtor_account_id",
                "creditor_account_id", "account_id"}
    assert required <= set(fields)
    for name, f in fields.items():
        assert f["classification"] in p1["classifications"], name
        for key in ("permitted_layers", "tokenisation", "masking", "logging", "consumption"):
            assert f[key], name
...
def test_restricted_fields_identified():
    ...
    for name in ("beneficiary_name", "nin", "phone", "beneficiary_id",
                 "x_beneficiary_sa", "debtor_account_id", "creditor_account_id"):
...
def test_keyed_tokenisation_deterministic_and_distinct(monkeypatch):
    ...
    assert dp.keyed_token("BEN-000001") != hashlib.sha256(b"BEN-000001").hexdigest()
...
def test_logs_do_not_expose_protected_values():
    clean = dp.redact_for_log("contact jane.doe@example.ug on +256772123456")
    assert "jane.doe@example.ug" not in clean and "+256772123456" not in clean
    assert "REDACTED" in clean
def test_payment_correlation_preserved_after_protection(monkeypatch):
    monkeypatch.setenv("DP_TOKEN_KEY", "gate2-test-key")
    pmn = {"correlation_id": "C-1", "uetr": "U-1", "x_beneficiary_sa": "SA-9"}
    plm = {"correlation_id": "C-1", "uetr": "U-1", "x_beneficiary_sa": "SA-9"}
    for key in ("correlation_id", "uetr"):
        assert pmn[key] == plm[key]
    assert dp.keyed_token(pmn["x_beneficiary_sa"]) == dp.keyed_token(plm["x_beneficiary_sa"])
    assert "SA-9" not in (dp.keyed_token("SA-9") or "")
def test_phase6_pmn_plm_semantics_intact():
    pmn = sql_text("bronze", "br_pdm_payments_pmn_lifecycle_events.sql")
    plm = sql_text("bronze", "br_pdm_payments_plm_lifecycle_events.sql")
    uni = sql_text("silver", "slv_pdm_payment_technical_events.sql")
    assert "x_beneficiary_sa" in plm and "x_wallet_reference" in plm
    assert "x_beneficiary_sa" not in pmn
    assert "x_beneficiary_sa" in uni and "technical_source" in uni
    assert "correlation_id" in uni and "uetr" in uni
def test_restricted_identity_fails_closed_without_authorisation():
    resolver = IdentityResolver(authorised_subjects={"governance-admin"})
    with pytest.raises(IdentityAccessDenied):
        resolver.resolve(subject="random-analyst", token="v1:abc", purpose="curiosity")
    assert resolver.audit_log and resolver.audit_log[0]["decision"] == "DENY"
def _gap_guard(model_rel, pattern):
    text = sql_text(*model_rel)
    assert not re.search(pattern, text), (
        "ENFORCEMENT GAP (STOP: needs approved Iceberg replacement): "
        + "/".join(model_rel) + " still matches " + pattern)
def test_gap_silver_clear_identity_requires_vault():
    _gap_guard(("silver", "slv_pdm_beneficiaries.sql"),
               r"(?i)\b(beneficiary_name|\bnin\b|date_of_birth)\b")
def test_gap_gold_no_direct_identifiers_or_plain_sha256():
    _gap_guard(("gold", "portfolio", "gld_dim_pdm_beneficiary.sql"), r"(?i)\bbeneficiary_name\b")
    _gap_guard(("gold", "portfolio", "gld_dim_pdm_beneficiary.sql"), r"sha256\s*\(")
def test_gap_consumption_no_beneficiary_identity():
    _gap_guard(("consumption", "leadership", "cns_pdm_beneficiary_insights.sql"), r"(?i)\bbeneficiary_id\b")
    _gap_guard(("consumption", "operations", "cns_pdm_payment_operations.sql"), r"(?i)\bbeneficiary_id\b")
    _gap_guard(("consumption", "investigators", "cns_pdm_ai_default_risk.sql"), r"(?i)\bbeneficiary_id\b")
def test_gap_trino_not_world_open():
    rules = (ROOT / "platform/trino/etc/rules.json").read_text()
    assert '"user": ".*"' not in rules, (
        "ENFORCEMENT GAP (STOP: approved Trino rules replacement needed)")

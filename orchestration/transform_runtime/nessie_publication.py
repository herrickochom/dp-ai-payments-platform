...
def branch_for_run(run_id):
    if not re.fullmatch(r"tr_[a-f0-9]{32}",run_id): raise NessiePublicationError("invalid transform run identity")
...
class NessiePublisher:
    def __init__(self,endpoint=None,token=None,timeout_seconds=10):
        ...
        self.token=token if token is not None else os.getenv("NESSIE_AUTH_TOKEN","")
